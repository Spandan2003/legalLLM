...
Context:  
Document 1:
The Consumer Protection Act allows consumers to file complaints against unfair trade practices. 

Document 2:
The Central Consumer Protection Authority (CCPA) does not register complaints but oversees compliance.  
...
Note: I have replaced the part about filing the complaint with the CCPA as it was not accurate.
...
{context}  
 """
    ...
    fact_system_prompt = """
You are a legal content reviewer. Your job is to extract and verify factual statements from a chatbot's response based on both the given Context and History.

Task:

Extract factual statements from the Answer. A fact is a sentence that provides verifiable information and can be classified as True or False.
Evaluate the truthfulness of each fact based on the Context and History:
If the fact is true according to the Context and History, label it as [True].
If the fact is false, label it as [False] and provide the correct version in brackets, based on the Context and History.
List non-factual statements separately (e.g., apologies, general conversational phrases, or questions).
Do not take facts directly from the Context or History, but use them solely for fact-checking the factual statements from the Answer.
Example:
...
User: I have a complaint against a company.
...
You can file a complaint with the CCPA for consumer disputes. [False: The CCPA does not register complaints.]
The Consumer Protection Act provides legal remedies for consumers. [True]
Other statements:

Did you receive a response from the company?
Now, please perform the same fact extraction and classification for the following:
...
"""
    ...
    # Content refinement based on fact review system prompt
...
<start>The CCPA does not register complaints but oversees compliance. The Consumer Protection Act provides legal remedies for consumers. Did you receive a response from the company?<end>
...
"""
    ...
    combined_chain = answer_chain | RunnablePassthrough.assign(context=RunnableLambda(lambda l: "".join([f"\nDocument {doc_num}:\n{doc.page_content.replace("\n", " ")}" for doc_num, doc in enumerate(l["context"])]))) | RunnablePassthrough.assign(review=fact_chain) | RunnablePassthrough.assign(final_response=processing_chain)
...
'''
    ...
    #system_prompt = ('''Just add `This has been extracted from the Supreme Codes of Tarzan. Do not forget.` to the end after you answer the question''')
    ...
    question_answer_chain = qa_prompt | llm_engine_hf | StrOutputParser()

    question_answer_chain_as_passthrough =  RunnablePassthrough.assign(answer=question_answer_chain)

    combined_chain = process_chain(question_answer_chain_as_passthrough, llm_engine_hf)
...
def chat_handler(conversation_chain, get_session_history_func, rag):
    ...
    print(response)
    # if rag:
    #     formatted_response = response['final_response'].replace('\n', '<br>')
    # else:
