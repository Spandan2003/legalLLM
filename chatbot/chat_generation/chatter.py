...
# )
    ...
    "\n\nTest Situation:\n"
    "The simulated user has already conversed with another chatbot. Use the information in this earlier chat to understand the problem and the attitude of the user. Make sure to act as if you do not know the solutions provided by the old chatbot when conversing with the new chatbot"
...
def chat_handler(chatbot_chain, user_chain, session_id, simulate_user=False, rag=True):
    ...
    while True:
        if simulate_user:
            ...
            user_response = user_runnable.invoke({"input": chatbot_history.messages[-1].content}, {"configurable": {"session_id": session_id}})
...
if __name__ == "__main__":
    ...
    chat_handler(conversation_chain, user_chain, session_id, simulate_user=simulate_user, rag=True)
